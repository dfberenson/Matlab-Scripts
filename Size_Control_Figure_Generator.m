...
for cond = 1:1
    ...
    % xlabel('prEF1a-mCherry-NLS at birth')
    % ylabel('G1 length (h)')
    ...
    ylabel('prEF1-mCherry-NLS at G1/S')
    %     yyaxis right
    ...
    %     ylabel('prEF1-mCherry-NLS accumulation during G1')
    xlabel('prEF1-mCherry-NLS at birth')
    ...
    xlabel('prEF1-mCherry-NLS at G1/S')
    ylabel('prEF1-mCherry-NLS at G2/M')
    ...
    xticks([0 1 2 3 4])
    yticks([0 10 20 30])
    xlabel('prEF1-mCherry-NLS at phase start')
    ylabel('Phase duration (h)')
    legend('G1','SG2')
    title(data(cond).treatment)
    hold off
    
    disp(['CoV at birth = ' num2str(std(birth_sizes_born_and_pass_g1s)/mean(birth_sizes_born_and_pass_g1s))])
    disp(['CoV at G1/S = ' num2str(std(g1s_sizes_born_and_pass_g1s)/mean(g1s_sizes_born_and_pass_g1s))])
    disp(['CoV at G2/M = ' num2str(std(g2m_sizes_divide)/mean(g2m_sizes_divide))])
end