...
measure_area_vs_fluorescence = false;
measure_sizes_at_birth_and_other_times = false;
measure_lengths_of_phases = false;
measure_protein_concentration_around_g1s = true;
...
if measure_protein_concentration_around_g1s
    for cond = 1:num_conditions
        
        data(cond).all_frame_indices_wrt_g1s = [];
        data(cond).all_g1s_happens_here = [];
        data(cond).all_areas_up_to_g1s = [];
        data(cond).all_sizes_up_to_g1s = [];
        data(cond).all_protein_amts_up_to_g1s = [];
        data(cond).all_protein_per_area_up_to_g1s = [];
        data(cond).all_protein_per_size_up_to_g1s = [];
        
        for pos = data(cond).positions_list
            for c = data(cond).position(pos).tracking_measurements.all_tracknums
                if data(cond).position(pos).analysis(c).passes_g1s
                    data(cond).all_frame_indices_wrt_g1s = [data(cond).all_frame_indices_wrt_g1s;...
                        data(cond).position(pos).analysis(c).frame_indices_wrt_g1s];
                    data(cond).all_g1s_happens_here = [data(cond).all_g1s_happens_here;...
                        data(cond).position(pos).analysis(c).g1s_happens_here];
                    data(cond).all_areas_up_to_g1s = [data(cond).all_areas_up_to_g1s;...
                        data(cond).position(pos).analysis(c).areas_up_to_g1s];
                    data(cond).all_sizes_up_to_g1s = [data(cond).all_sizes_up_to_g1s;...
                        data(cond).position(pos).analysis(c).sizes_up_to_g1s];
                    data(cond).all_protein_amts_up_to_g1s = [data(cond).all_protein_amts_up_to_g1s;...
                        data(cond).position(pos).analysis(c).protein_amt_up_to_g1s];
                    data(cond).all_protein_per_area_up_to_g1s = [data(cond).all_protein_per_area_up_to_g1s;...
                        data(cond).position(pos).analysis(c).protein_per_area_up_to_g1s];
                    data(cond).all_protein_per_size_up_to_g1s = [data(cond).all_protein_per_size_up_to_g1s;...
                        data(cond).position(pos).analysis(c).protein_per_size_up_to_g1s];
                end
            end
        end
    end
...
if measure_protein_concentration_around_g1s
    all_data_types_to_plot = {'Relative_frame_vs_G1S_probability','Area_vs_G1S_probability',...
        'Size_vs_G1S_probability','Rb_amt_vs_G1S_probability','Rb_per_area_vs_G1S_probability',...
        'Rb_per_size_vs_G1S_probability'};
    all_cell_classes_to_plot = {'all'};
    all_plottypes = {'logit'};
            ...
            for plottype = all_plottypes
                for cond = 1:num_conditions
                    ...
                    if strcmp(data_type_to_plot,'Relative_frame_vs_G1S_probability')
                        ...
                        if strcmp(cell_class_to_plot,'all')
                            data_to_scatter(cond).x = data(cond).all_frame_indices_wrt_g1s;
                            data_to_scatter(cond).y = data(cond).all_g1s_happens_here;
                            graph_title = [data(cond).treatment ', all cells that pass G1/S'];
                            is_there_something_to_plot = true;
                        end
                    ...
                    elseif strcmp(data_type_to_plot,'Area_vs_G1S_probability')
                        ...
                        if strcmp(cell_class_to_plot,'all')
                            data_to_scatter(cond).x = data(cond).all_areas_up_to_g1s;
                            data_to_scatter(cond).y = data(cond).all_g1s_happens_here;
                            graph_title = [data(cond).treatment ', all cells that pass G1/S'];
                            is_there_something_to_plot = true;
                        end
                    ...
                    elseif strcmp(data_type_to_plot,'Size_vs_G1S_probability')
                        ...
                        if strcmp(cell_class_to_plot,'all')
                            data_to_scatter(cond).x = data(cond).all_sizes_up_to_g1s;
                            data_to_scatter(cond).y = data(cond).all_g1s_happens_here;
                            graph_title = [data(cond).treatment ', all cells that pass G1/S'];
                            is_there_something_to_plot = true;
                        end
                    ...
                    elseif strcmp(data_type_to_plot,'Rb_amt_vs_G1S_probability')
                        ...
                        if strcmp(cell_class_to_plot,'all')
                            data_to_scatter(cond).x = data(cond).all_protein_amts_up_to_g1s;
                            data_to_scatter(cond).y = data(cond).all_g1s_happens_here;
                            graph_title = [data(cond).treatment ', all cells that pass G1/S'];
                            is_there_something_to_plot = true;
                        end
                    ...
                    elseif strcmp(data_type_to_plot,'Rb_per_area_vs_G1S_probability')
                        ...
                        if strcmp(cell_class_to_plot,'all')
                            data_to_scatter(cond).x = data(cond).all_protein_per_area_up_to_g1s;
                            data_to_scatter(cond).y = data(cond).all_g1s_happens_here;
                            graph_title = [data(cond).treatment ', all cells that pass G1/S'];
                            is_there_something_to_plot = true;
                        end
                    ...
                    elseif strcmp(data_type_to_plot,'Rb_per_size_vs_G1S_probability')
                        ...
                        if strcmp(cell_class_to_plot,'all')
                            data_to_scatter(cond).x = data(cond).all_protein_per_size_up_to_g1s;
                            data_to_scatter(cond).y = data(cond).all_g1s_happens_here;
                            graph_title = [data(cond).treatment ', all cells that pass G1/S'];
                            is_there_something_to_plot = true;
                        end
