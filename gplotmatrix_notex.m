function [h,ax,BigAx] = gplotmatrix_notex(x,y,g,clr,sym,siz,doleg,dispopt,xnam,ynam)
...
if (donames && ~isempty(xnam))
   for j=1:cols
      ...
      h = text(0.5,0.5, -.1,...
          ...
          'VerticalAlignment','middle','Units','normalized','Interpreter','none');
   end
else
   if ~isempty(xnam)
      for j=1:cols, xlabel(ax(rows,j),xnam{j},'Interpreter','none'); end
   end
   if ~isempty(ynam)
      for i=1:rows, ylabel(ax(i,1),ynam{i},'Interpreter','none'); end
   end
end
...
if ~isempty(gcbf)
    % Make sure hz1 and ax2 have same figure parent. Using get(groot,'CurrentFigure')
    % is okay because this gplotmatrix creates a new plot where ax2 comes
    % from. But using ax2(1).Parent directly would cause problem in test
    % cases ax2 was not available here.
